...
try:
    ...
    with conn.cursor() as cursor:
        sql= CREATE TABLE Attack(
                CNe int(10) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                IDX int(10)
    ...
    while 1:
        ...
        try:
            conn = pymysql.connect(host='localhost',
                    ...
                    charset='utf8mb4')
            with conn.cursor() as cursor:
                cursor.execute("select count(*) from Attack")
                row_result = cursor.fetchone()
                number_of_rows = row_result[0]

                number_of_rows+=1
                print(number_of_rows)
                sql = "INSERT INTO Attack (CNT,IDX) VALUES("+ str(number_of_rows) + "," + str(result[0]) + ")"
                cursor.execute(sql)
