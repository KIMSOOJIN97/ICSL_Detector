...
type Mysql struct {
	...
	GatherTableData                     bool     `toml:"gather_table_data"`
...
/////////////////////////////////////SOOJIN/////////////////////////////////////////////////    
	tableSchemaData = `
	SELECT L1MISS, L2MISS, L3MISS, BRANCH
	FROM test
	WHERE cnt = %d
    `
...
///////////////////////////////////////SOOJIN////////////////////////////////////////////
	if m.GatherTableData {
		err = m.gatherTableData(db, serv, acc)
		if err != nil {
			return err
		}
	}
...
///////////////////////////////////////////////soojin//////////////////////////////////
func (m *Mysql) gatherTableData(db *sql.DB, serv string, acc telegraf.Accumulator) error {
	...
	for{
		...
		s1=fmt.Sprintf(tableSchemaData,counter_cnt)
		s2=fmt.Sprintf(AttackData,idx_cnt)
		var(
			l1      int64
			l2      int64
			l3      int64
		...
		tags := map[string]string{"server": servtag}
		err1 := db.QueryRow(s1).Scan(
			&l1,
			&l2,
			&l3,
			&br,
		)
		err2 := db.QueryRow(s2).Scan(
			&cnt,
			&idx,
		)
		...
		acc.AddFields("mysql_table_data",fields_1,tags)
